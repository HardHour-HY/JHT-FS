...
export async function exportModeSummary(batch:Batch,products:Product[]){
  ...
  for(const {row,product} of designDetails.filter(item=>item.product.layoutType==='设计排版')){const key=`${product.layoutType}\u0000${row.sku}`,prior=designSkuTotals.get(key);if(prior)prior.quantity+=row.quantity;else designSkuTotals.set(key,{layoutType:product.layoutType,sku:row.sku,quantity:row.quantity});}
