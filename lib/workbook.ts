...
export async function exportProductTemplate(categories: string[] = [], modes: string[] = []) {
  ...
  guide.addRows([
    ...
    ['必填字段','产品货号、产品模式、排版分类、产品分类。非定制模式的非设计排版商品还必须填写源路径和目标路径；产品名称可留空，填写后不能与其他商品重复。产品货号必须唯一。'],
    ['排版分类','只能填写“设计排版”或“非设计排版”，必须二选一。设计排版且不需要打印的商品可以不填写路径。'],
    ['是否需要打印','只能填写“是”或“否”；产品模式为“定制”时可以不填写路径。其他设计排版且需要打印的商品仍须填写路径。新商品不填写时按“否”处理。'],
...
export async function exportModeSummary(batch:Batch,products:Product[]){
  ...
  const paperSheet=book.addWorksheet('纸质产品'),paperDetails=batch.details.flatMap(row=>{const product=lookup.get(productCode(row.sku));return product?.printRequired&&product.mode!=='定制'?[{row,product}]:[];}).sort((a,b)=>[a.product.category,a.row.warehouse,a.row.packageNo,a.row.sku].join('\u0000').localeCompare([b.product.category,b.row.warehouse,b.row.packageNo,b.row.sku].join('\u0000'),'zh-CN'));
  ...
  const calendarSheet=book.addWorksheet('定制挂历'),calendarDetails=batch.details.flatMap(row=>{const product=lookup.get(productCode(row.sku));return product?.mode==='定制'&&product.printRequired?[{row,product}]:[];}).sort((a,b)=>[a.product.category,a.row.warehouse,a.row.packageNo,a.row.sku,a.row.customId||''].join('\u0000').localeCompare([b.product.category,b.row.warehouse,b.row.packageNo,b.row.sku,b.row.customId||''].join('\u0000'),'zh-CN'));
  ...
  const designSheet=book.addWorksheet('设计作图产品'),designDetails=batch.details.flatMap(row=>{const product=lookup.get(productCode(row.sku));return product?[{row,product}]:[];});
  const designHeaders=(column:number,headers:string[])=>headers.forEach((header,index)=>{const cell=designSheet.getCell(1,column+index);cell.value=header;cell.font={bold:true,color:{argb:'FFFFFFFF'}};cell.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FF276447'}};});
  const designTotal=(row:number,column:number,values:(string|number)[])=>values.forEach((value,index)=>{const cell=designSheet.getCell(row,column+index);cell.value=value;cell.font={bold:true};cell.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FFE8F1EC'}};});
  const mergeDesign=(rows:(string|number)[][],startColumn:number,dimensionColumns:number)=>{for(let dimension=0;dimension<dimensionColumns;dimension++){let from=0;while(from<rows.length){let to=from+1;while(to<rows.length&&rows[to].slice(0,dimension+1).every((value,index)=>value===rows[from][index]))to++;if(to-from>1){designSheet.mergeCells(from+2,startColumn+dimension,to+1,startColumn+dimension);designSheet.getCell(from+2,startColumn+dimension).alignment={vertical:'middle'};}from=to;}}};
  designHeaders(1,['产品分类','SKU货号','发货数']);designHeaders(5,['产品货号','SKU货号','定制ID','发货数']);designHeaders(10,['产品货号','SKU货号','定制ID','收货仓库','发货数']);
  const designSkuTotals=new Map<string,{layoutType:string;sku:string;quantity:number}>();
  for(const {row,product} of designDetails.filter(item=>item.product.layoutType==='设计排版'&&item.product.mode!=='定制')){const key=`${product.layoutType}\u0000${row.sku}`,prior=designSkuTotals.get(key);if(prior)prior.quantity+=row.quantity;else designSkuTotals.set(key,{layoutType:product.layoutType,sku:row.sku,quantity:row.quantity});}
  const designRows=[...designSkuTotals.values()].sort((a,b)=>[a.layoutType,a.sku].join('\u0000').localeCompare([b.layoutType,b.sku].join('\u0000'),'zh-CN')).map(item=>[item.layoutType,item.sku,item.quantity] as (string|number)[]);
  designRows.forEach((values,index)=>values.forEach((value,column)=>designSheet.getCell(index+2,column+1).value=value));mergeDesign(designRows,1,2);designTotal(designRows.length+2,1,['合计','',designRows.reduce((sum,row)=>sum+Number(row[2]),0)]);
  const customRows=(printRequired:boolean,withWarehouse:boolean)=>{const totals=new Map<string,{code:string;sku:string;customId:string;warehouse:string;quantity:number}>();for(const {row,product} of designDetails.filter(item=>item.product.mode==='定制'&&Boolean(item.product.printRequired)===printRequired)){const customId=row.customId?.trim();if(!customId)throw new Error('定制模式商品缺少定制ID，请重新上传并选择“定制ID”列');const key=[product.code,row.sku,customId,withWarehouse?row.warehouse:''].join('\u0000'),prior=totals.get(key);if(prior)prior.quantity+=row.quantity;else totals.set(key,{code:product.code,sku:row.sku,customId,warehouse:row.warehouse,quantity:row.quantity});}return [...totals.values()].sort((a,b)=>[a.code,a.sku,a.customId,withWarehouse?a.warehouse:''].join('\u0000').localeCompare([b.code,b.sku,b.customId,withWarehouse?b.warehouse:''].join('\u0000'),'zh-CN')).map(item=>withWarehouse?[item.code,item.sku,item.customId,item.warehouse,item.quantity]:[item.code,item.sku,item.customId,item.quantity]);};
  const customNoPrintRows=customRows(false,false),customPrintRows=customRows(true,true);
  ...
  const orderSheet=book.addWorksheet('下载订单与合并订单'),orderDetails=designDetails.filter(item=>item.product.mode!=='定制'&&(!item.product.printRequired||(item.product.printRequired&&item.product.layoutType==='设计排版')));
