...
export function parseProducts(rows: string[][], headerRow: number, existing: Product[], aliases: Alias[], idFactory=()=>crypto.randomUUID()): ProductImportResult {
  ...
  rows.slice(headerRow+1).forEach((row,offset)=>{
    ...
    const old=existingMap.get(code); const base={id:old?.id||idFactory(),code,name,mode:productMode,layoutType:layoutType as Product['layoutType'],category,printRequired,writeAccessories:old?.writeAccessories??false,accessories:old?.accessories??[],subcategory:old?.subcategory||'',image,imageKey:image?'':old?.imageKey??'',note};
