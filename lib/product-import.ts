import type { Alias, PathValue, Product } from './shipping';
...
export function parseProducts(rows: string[][], headerRow: number, existing: Product[], aliases: Alias[], idFactory=()=>crypto.randomUUID()): ProductImportResult {
  ...
  rows.slice(headerRow+1).forEach((row,offset)=>{
    ...
    if(prior){const same=['name','mode','category','subcategory','image','note'].every(k=>prior.base[k as keyof typeof prior.base]===base[k as keyof typeof base]);if(!same)issues.push(`第 ${line} 行：产品 ${code} 的资料与第 ${prior.firstLine} 行不一致`);prior.paths.push({source,target});}
    else grouped.set(code,{base,paths:[{source,target}],firstLine:line});
  });
